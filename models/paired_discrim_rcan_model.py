...
class PairedDiscrimRCANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators
            ...
            self.fake_pool_real = ImagePool(opt.pool_size)  # create image buffer to store previously generated images
            self.fake_pool_random = ImagePool(opt.pool_size) 
    ...
    def backward_D(self):
        ...
        # (self.random, self.canonical_pred_random) Fake
        # (self.real, self.canonical_pred_real) Fake
        fake_canonical_random = self.fake_pool_random.query(self.random_pair_pred)
        fake_canonical_real = self.fake_pool_real.query(self.real_pair_pred)

        self.loss_D = self.backward_D_basic(self.netD, self.random_pair_true, fake_canonical_random, fake_canonical_real)

    def backward_G(self):
        """Calculate the loss for generators G_canonical and G_pred"""

        self.loss_discrim = self.criterionGAN(self.netD(self.random_pair_pred), True)
        ...
        self.loss_discrim_real = self.criterionGAN(self.netD(self.real_pair_pred), True)
